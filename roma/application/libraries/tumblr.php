...
class Tumblr {	
    ...
    function read_posts() 
    {
    	...
    	$posts = $this->parse_posts_full($this->tumblr_blog_url.'/api/read', implode("&", $params), $method);
    ...
    */
    function parse_posts_full($url, $data="", $method="POST")
    {
    	...
    	if (!$xml) { // error in parsing XML
    		...
    		$posts = $this->objectsIntoArray($xml);
